                ...
                if ($this->widgets_type == AmosWidgets::TYPE_ICON) {
                    ...
                    if ($widgets->count()) {
                        foreach ($widgets->all() as $value) {
                            if (empty(\Yii::$app->params['isPoi']) || (!empty(\Yii::$app->params['isPoi']) && \Yii::$app->params['isPoi']
    ...
    protected function runHtml($widgets)
    {
        ...
        if ($widgets->count()) {
            if ($this->graphicCustomSize) {
                foreach ($widgets->all() as $value) {
                    if($this->model->className() != \lispa\amos\community\models\Community::className()
                                   || ($value->classname !=  \lispa\amos\documenti\widgets\graphics\WidgetGraphicsUltimiDocumenti::className())
                                    || ($value->classname ==  \lispa\amos\documenti\widgets\graphics\WidgetGraphicsUltimiDocumenti::className()
                                           && ($this->model->id != 2751)))
                            {
